...
summary는 위 분량을 유지한 채 2~4개 문단으로 나누고, 문단 사이는 빈 줄(\\n\\n)로 구분한다. 각 문단은 \`**앵커.**\` 형식(마크다운 볼드 + 마침표)으로 시작하고 공백 하나 뒤에 본문을 이어 쓴다.
앵커 규칙:
- 그 문단의 내용을 압축한 2~10자의 명사구 또는 짧은 구절. 영상 주제·장르에 맞게 매번 새로 만들 것(고정 라벨 반복 금지).
...
keyPoints는 근거 3~5개(논점이 적으면 3개, 많으면 5개까지. 억지로 5개를 채우지 말 것). 제공된 자막에는 [m:ss] 형식의 실제 시간 앵커가 포함되어 있다. timeline의 time은 반드시 자막에 실제로 등장한 [m:ss] 시각만 사용하고 창작 금지. 앵커 없으면 timeline은 []. timeline은 의미 있는 구간 4~6개, 각 content는 그 구간 주제.`
    ...
    lengthGuide = "summary는 3~4문장. summary는 위 분량을 유지한 채 1~2개 문단으로 나누고, 문단 사이는 빈 줄(\\n\\n)로 구분한다. 각 문단은 `**앵커.**` 형식(마크다운 볼드 + 마침표)으로 시작하고 공백 하나 뒤에 본문을 이어 쓴다.\n앵커 규칙:\n- 그 문단의 내용을 압축한 2~10자의 명사구 또는 짧은 구절. 영상 주제·장르에 맞게 매번 새로 만들 것(고정 라벨 반복 금지).\n- 문단이 둘이면 서로 다른 층위를 담을 것(무엇을 다루는지 / 원인·방법 / 의미·파장 / 결론·조언 중 영상 성격에 맞게).\n- '요약', '내용', '설명', '정리' 같은 일반어 금지. 제목이나 tldr 문구를 그대로 반복하지 말 것.\nkeyPoints는 3~4개, '핵심 내용 — 부연' 형태. timeline은 빈 배열 []."
  } else {
...
[전체 구조 — 세 층이 겹치지 않고 점점 깊어지게]
독자는 tldr → keyPoints → summary 순으로 읽으며, 어느 층에서 멈춰도 완결된 정보를 얻어야 합니다.
각 층은 아래층의 예고편 역할을 하되 같은 문장을 반복하지 않습니다.
  1. tldr(결론) — 제목의 주장·질문에 대한 답. 결론과 그것을 지탱하는 핵심 근거 한 조각을 한 문장에 담는다.
     예: "장부가 90만 원에 보수적 PBR 2배를 적용한 결과이며, 과거 급락-급등 패턴상 현재가 저평가 구간이라는 논리."
  2. keyPoints(근거) — 그 결론을 뒷받침하는 서로 다른 근거를 나열한다. 결론의 재진술이 아니라 근거여야 한다.
  3. summary(논리 전개) — 근거들이 어떻게 연결되어 결론에 이르는지를 서술한다. 배경, 인과, 이번 사례가 과거와 다른 점.
- **tldr에 이미 쓴 내용은 keyPoints의 어느 항목에도 반복하지 말 것**(첫 항목뿐 아니라 전 항목).
  tldr은 결론과 근거 한 조각을 담으므로, 그 근거 조각은 keyPoints에서 빼고 **다른 근거들로 채워라.**
  keyPoints를 다 쓴 뒤 tldr과 나란히 놓고 읽어, 같은 사실을 말하는 항목이 하나라도 있으면 그 항목을
  아직 다루지 않은 논점으로 교체하라. tldr은 "그래서 결론이 무엇인가", keyPoints는 "그 외에 또 무슨 근거가 있는가"이다.
- **keyPoints끼리도 서로 다른 논점이어야 한다.** 같은 사실을 표현만 바꿔 두 항목으로 쪼개지 말 것.
...
- **summary는 논리 전개다 — keyPoints를 문장으로 늘려 쓴 글이 아니다.**
  ...
  · **결론을 말하는 문장은 summary 전체에서 한 번만.** 마지막에 같은 결론을 두세 문장으로 되풀이하며 끝내지 말 것. 분량이 남으면 결론을 반복하지 말고 다루지 않은 논점을 추가하라.
  · 단, summary는 그 자체로 완결된 글이어야 한다. 개괄만 남기고 디테일을 keyPoints로 넘기지 말고, 수치와 고유명사는 summary 안에서 근거로 충분히 활용할 것.
