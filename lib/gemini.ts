...
- tldr은 "제목만 보고는 알 수 없는, 영상이 실제로 밝히는 핵심 답"을 한 문장으로(공백 포함 80자 이내). 제목이 던진 질문이나 예고에 대한 '구체적 답·수치·결론'을 담아 제목과 차별화할 것. 제목/summary 첫 문장을 그대로 반복하지 말 것. 예: 제목이 "금리 인하가 부동산에 미치는 영향 3가지"라면 tldr은 "금리 인하는 부동산에 영향을 준다"(제목 반복·무의미)가 아니라 "3가지 중 전세가 상승이 가장 빠르게 나타난다"(제목이 안 준 구체적 답)처럼 작성. 영상 내용에 그런 구체적 답이 없으면 억지로 만들지 말고 가장 정보가치 높은 사실 한 가지를 담을 것. 다국어 요약 시 tldr도 해당 언어.
