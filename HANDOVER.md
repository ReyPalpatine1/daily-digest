...
## 15. 백로그 (2026-06-30 기준)

> 현재 작업 기준. 완료되면 ④로 옮기고, 새 항목은 분류에 맞게 추가할 것.

### ① 출시 전 필수
- 요금제·결제 페이지/UI (실결제 PG 연동은 사업자 등록 후로 보류)
- 약관·개인정보처리방침·환불 규정
- 탈퇴(회원 탈퇴) 기능 — 계정·데이터 삭제. 현재 코드에 없음, 신규 구현
- 안되는 항목 사유 표기 (대시보드/사이트) — 요약 실패 시 사유 구분 표시: 자막 없음 / 요약 3회 실패 등. (멤버십·라이브 영상 감지는 별도 작업, 이메일 표시는 ②백로그)
- $5 Cloudflare Workers Paid + Supabase Pro $25 여부 결정 (Supabase는 무료로 버틸 수 있는지 사용량 점검 먼저) → 전환 시 MAX_SUMMARIES_PER_RUN 1→100
- 자체 백업 (pg_dump 오프사이트 R2/S3/GitHub + 실패 알림 + 빈도 상향) — 유료 고객 데이터까지 커버. Supabase 무료 tier는 백업 보존 0일이라 안전망 필요

### ① 확정 정책 (요금제)
- 가격: Pro ₩4,900/월·₩39,000/년(≈33%↓) / 무료 ₩0
- 채널: 무료 5 / Pro 20 (무제한 폐기)
- 자막 없는 영상 AI 요약: Pro 전용 (무료는 자막 있는 영상만)
- 속보: Pro 전용, 문구 "실시간 속보" (분 단위 숫자 표기 안 함, cron 15분이라)
- 발송 수단: 무료 이메일 / Pro +텔레그램
- 기록: 무료 7일 열람(저장 30일) / Pro 30일
- 광고/제휴: 무료 있음(광고 넣기 확정) / Pro 없음
- 다국어 요약: 무료·Pro 공통(차별점 아님)
- VIP: 관리자가 무료 Pro 부여. 사용자 화면 "PRO" 표시, 관리자 페이지에서만 VIP 구분
- 로그인: Google OAuth만

### ② 보류 (방향 잡힘, 시점·세부 미정)
- 카카오/WhatsApp/LINE 메신저 (출시 후, 현재 텔레그램만)
- 일회성 결제(1개월권/1년권) — 결제 페이지 만들 때 결정
- 7일 무료 체험 — 실결제 붙일 때 결정
- 소셜 로그인 추가(카카오 등) — 자체 이메일+비번은 비추
- Gemini 타임아웃 대응(thinking 최소화+45s+재시도) — 재발 시
- 멤버십/라이브 전용 영상 감지·스킵
- digests 정규화 (summary 복사 → video_id 참조+JOIN)
- 관리자 화면 자막 비용 계산 (transcriptapi×$1.5/1k + supadata×$9/1k + Gemini 토큰)
- 언어 동기화 로직 중복 통합 (대시보드+AppHeader)
- subrequest 근본 대응: 수집/요약 별도 실행 분리 + 영상단위 큐화
- 요약 실패 사유를 이메일에도 표시 (대시보드 먼저, 이메일은 나중)
- 제휴 시스템 (영상 주제별 타겟 광고/제휴 — 수익화 핵심, 사용자 기반 생긴 뒤)

### ③ 아이디어만 (세부 미정)
- 이주의 인기 영상 위젯 (조회수 기반, "인기" 라벨, AI 비용 0)
- 북마크(★)·나중에 볼 영상 (기록 보관 차등 미정)
- "지금 요약하기" → "오늘 미리보기" (메일 미발송+대시보드 렌더+캐시 / 관리자 실발송 분리, 세부 6가지 미정)
- 폐기됨(참고): 영상 정밀 요약, 매일 여러 번 다이제스트, 우선 처리, Premium/Family, 슈퍼 프로 무제한, AI 음성 요약, 디스코드, 카테고리 수 제한

### ③ UI·기능 다듬기 (출시 전)
- 랜딩 페이지 디자인 다듬기 (카피·레이아웃·색 미세조정)
- 도움말 모달 디자인 다듬기
- 요약 메일 다듬기 (속보 배너 수정 + 요약 내용 중복 등)

### ④ 완료 (확정 중 완료)
- 구조3 자막 라인 (TranscriptAPI 1차 + Supadata 폴백 + 설명 폴백) — 로그 검증 완료
- Cloudflare Cron Triggers 이전 (worker.ts scheduled: collect→cron) — :00 자동 발화 확인 (정시성 개선은 운영 관찰 대기)
- 랜딩 페이지 (키커+헤드라인+메일 미리보기 가상 채널 4개+페이드)
- 전역 색 톤 중립화 (globals.css 변수만)
- 도움말 팝업 (720×680 캐러셀 3스텝, help_seen DB 양방향 연동, 설정 재열기)
- MAX_SUMMARIES_PER_RUN=1 (베타 임시, subrequest 50 한도 대응)

### 출시 전 잡일
- 이모지→lucide 아이콘 교체
- 영어 이메일 재검증
- 미사용 i18n 키 정리 (단, pricing이 쓰는 faq 키는 건드리지 말 것)
- Vercel 정리 (Cloudflare 안정 확인 후)
